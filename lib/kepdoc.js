...
/* ---------- Fusion d'un modele Word (champs \u00abX\u00bb, lignes de tableau \u00abCODE\u00bb, puces \u00abPUCE\u00bb), comme les courriers BTS ---------- */
async function fusionDocx(buf,o={}){await zipLib();const Z=await JSZip.loadAsync(buf),W="http://schemas.openxmlformats.org/wordprocessingml/2006/main";
  const d=new DOMParser().parseFromString(await Z.file("word/document.xml").async("string"),"application/xml");
  const all=(el,n)=>Array.from(el.getElementsByTagNameNS(W,n));
  ...
  if(proto){for(const l of (o.lignes||[])){const tr=proto.cloneNode(true);for(const k of Object.keys(l))setT(tr,"\u00ab"+k+"\u00bb",l[k]==null?"":String(l[k]));proto.parentNode.insertBefore(tr,proto);}proto.parentNode.removeChild(proto);}
  ...
  let x=new XMLSerializer().serializeToString(d);
  for(const [k,v] of Object.entries(o.champs||{}))x=x.split("\u00ab"+k+"\u00bb").join(xesc(v==null?"":String(v)));
  x=x.replace(/\u00ab[A-Z0-9]+\u00bb/g,"");const th=o.theme||{fill:"FFFFFF",color:"000000"};
  x=x.split("HFILL").join(th.fill==="FFFFFF"?"EEF3EF":th.fill).split("HCOLOR").join(th.color);
  Z.file("word/document.xml",x);return Z.generateAsync({type:"uint8array",compression:"DEFLATE"});}
/* ---------- Utilitaires ---------- */
...
window.KepDoc={xlsx,pdf,courriers,qualiopi,qualiopiDocx,fusionDocx,zip,telecharger,nomFichier,chargerZip:zipLib};
})();
