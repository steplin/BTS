...
async function courriers(docs,o={}){await pdfLib();const {PDFDocument,StandardFonts,rgb:RGB}=PDFLib;
  ...
  for(const d of docs){let page=doc.addPage([W,H]),y=H-MT;
    ...
    const top=y;if(logo){const lw=119,lh=lw*logo.height/logo.width;page.drawImage(logo,{x:ML,y:top-lh,width:lw,height:lh});
      const s=propre(o.etab||"UFA Kerplouz LaSalle Auray");page.drawText(s,{x:ML+2,y:top-lh-10,size:8.5,font:R});}
    ...
    y=Math.min(top-(logo?70:0),yd)-14;
...
window.KepDoc={xlsx,pdf,courriers,zip,telecharger,nomFichier,chargerZip:zipLib};
})();
