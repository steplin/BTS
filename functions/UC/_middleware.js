// Protège /UC/ (saisie des documents 4 et 5 en UC) : comptes actifs ayant le droit « saisie-uc » (vérifié côté serveur).
const SB = "https://olqclfsywasktwirswpb.supabase.co";
const KEY = "sb_publishable_Qry6l1DGbG9IkLtGT7CF4w_OyP4rI8x";
const DROITS = ["saisie-uc"];
...
export async function onRequest(ctx) {
  ...
  if (m) {
    ...
    if (r.ok) {
      ...
      if (!ok && d && d.actif) {
        // responsable d'une épreuve sans être affecté à une classe du diplôme
        const r2 = await fetch(SB + "/rest/v1/rpc/a_droit_uc", {
          method: "POST",
          headers: { apikey: KEY, Authorization: "Bearer " + m[1], "Content-Type": "application/json" },
          body: "{}",
        });
