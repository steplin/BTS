// Protège /CCF/ (tableau de bord CCF) : comptes actifs ayant le droit « ccf-bts » ou « grilles-bts » (vérifié côté serveur).
const SB = "https://olqclfsywasktwirswpb.supabase.co";
const KEY = "sb_publishable_Qry6l1DGbG9IkLtGT7CF4w_OyP4rI8x";
const DROITS = ["ccf-bts", "grilles-bts"];
...
export async function onRequest(ctx) {
  ...
  if (m) {
    ...
    if (r.ok) {
      ...
      const ok = d && d.actif && (d.admin || DROITS.some((x) => (d.outils || []).includes(x)));
      if (ok) {
